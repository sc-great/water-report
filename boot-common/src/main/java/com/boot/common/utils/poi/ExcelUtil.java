...
import com.boot.common.annotation.Excel.ColumnType;
import com.boot.common.annotation.Excel.Type;
import com.boot.common.annotation.Excels;
...
import com.boot.common.utils.reflect.ReflectUtils;
...
public class ExcelUtil<T>
{
            ...
            for (int i = 1; i < rows; i++)
            {
                ...
                for (Map.Entry<Integer, Field> entry : fieldsMap.entrySet())
                {
                    ...
                    if (StringUtils.isNotNull(fieldType))
                    {
                        ...
                        if (StringUtils.isNotNullAndNotEmpty(val)) {
                            isAdd = true;
                        }
                    }
                }
    ...
    public AjaxResult exportExcel() {
        ...
        try {
            ...
            for (int index = 0; index <= sheetNo; index++) {
                ...
                int column = 0;
                // 写入各个字段的列头名称
                Row columnRow = sheet.createRow(isTitle ? 1 : 0);
                for (Object[] os : fields) {
                    Excel excel = (Excel) os[1];
                    this.createCell(excel, columnRow, column++);
                }
    ...
    public void fillExcelData(int index) {
        ...
        {
            Row row = sheet.createRow(i + (isTitle ? 2 : 1) - startNo);
            // 得到导出对象.
            T vo = (T) list.get(i);
            int column = 0;
            for (Object[] os : fields)
    ...
    public Cell createTitle(Row row) {
        CellStyle style = wb.createCellStyle();
        style.setAlignment(HorizontalAlignment.CENTER);
        style.setVerticalAlignment(VerticalAlignment.CENTER);
        Font ztFont = wb.createFont();
        ztFont.setFontName("Arial");
        ztFont.setFontHeightInPoints((short) 10);
        ztFont.setBold(true);
        ztFont.setColor(IndexedColors.BLACK.getIndex());
